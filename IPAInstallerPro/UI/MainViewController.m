...
- (void)setupDashboardHeader {
    ...
    self.dashboardHeader = [[UIView alloc] initWithFrame:CGRectMake(0, 0, width, 232.0)];
    ...
    UILabel *title = [[UILabel alloc] initWithFrame:CGRectMake(20, 27, width - 40, 40)];
    ...
    self.searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(116, 91, MAX(width - 136, 1), 42)]; self.searchBar.autoresizingMask = UIViewAutoresizingFlexibleWidth; self.searchBar.placeholder = @"البحث في الملفات..."; self.searchBar.searchBarStyle = UISearchBarStyleMinimal; self.searchBar.tintColor = [UIColor colorWithRed:1 green:.22 blue:.18 alpha:1]; self.searchBar.semanticContentAttribute = UISemanticContentAttributeForceRightToLeft; [self.dashboardHeader addSubview:self.searchBar];
    UIButton *filter = [UIButton buttonWithType:UIButtonTypeSystem]; filter.frame = CGRectMake(24, 94, MIN(88, MAX(width - 140, 72)), 38); filter.layer.cornerRadius = 14; filter.layer.borderWidth = 1; filter.layer.borderColor = [UIColor colorWithWhite:1 alpha:.12].CGColor; [filter setTitle:@"☷  تصفية" forState:UIControlStateNormal]; filter.titleLabel.font = [UIFont systemFontOfSize:14 weight:UIFontWeightSemibold]; [filter setTitleColor:[UIColor colorWithRed:1 green:.28 blue:.23 alpha:1] forState:UIControlStateNormal]; [self.dashboardHeader addSubview:filter];
    UIView *stats = [[UIView alloc] initWithFrame:CGRectMake(20, 139, MAX(width - 40, 1), 88)]; stats.autoresizingMask = UIViewAutoresizingFlexibleWidth; stats.backgroundColor = [UIColor colorWithRed:.065 green:.066 blue:.075 alpha:1]; stats.layer.cornerRadius = 20; stats.layer.borderWidth = 1; stats.layer.borderColor = [UIColor colorWithRed:.42 green:.08 blue:.09 alpha:.65].CGColor; [self.dashboardHeader addSubview:stats];
    ...
    for (NSInteger i = 0; i < 4; i++) { CGFloat x = stats.bounds.size.width / 4.0 * i; if (i) { UIView *d = [[UIView alloc] initWithFrame:CGRectMake(x, 18, 1, 52)]; d.backgroundColor = [UIColor colorWithWhite:1 alpha:.08]; [stats addSubview:d]; } UIImageView *iv = [[UIImageView alloc] initWithFrame:CGRectMake(x + (stats.bounds.size.width / 4.0 - 26) / 2.0, 9, 26, 26)]; iv.image = [UIImage systemImageNamed:icons[i]]; iv.tintColor = [UIColor colorWithRed:1 green:.22 blue:.18 alpha:1]; iv.contentMode = UIViewContentModeScaleAspectFit; [stats addSubview:iv]; UILabel *v = [[UILabel alloc] initWithFrame:CGRectMake(x + 3, 35, stats.bounds.size.width / 4.0 - 6, 24)]; v.textAlignment = NSTextAlignmentCenter; v.font = [UIFont systemFontOfSize:17 weight:UIFontWeightBold]; v.textColor = UIColor.whiteColor; [stats addSubview:v]; [values addObject:v]; UILabel *c = [[UILabel alloc] initWithFrame:CGRectMake(x + 1, 61, stats.bounds.size.width / 4.0 - 2, 18)]; c.text = labels[i]; c.textAlignment = NSTextAlignmentCenter; c.font = [UIFont systemFontOfSize:10 weight:UIFontWeightMedium]; c.textColor = [UIColor colorWithWhite:.68 alpha:1]; [stats addSubview:c]; }
