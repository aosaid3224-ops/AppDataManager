...
- (void)setupDashboardHeader {
    ...
    UILabel *title = [[UILabel alloc] initWithFrame:CGRectMake(24, 27, width - 48, 40)];
    ...
    self.searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(124, 91, MAX(width - 148, 170), 42)]; self.searchBar.autoresizingMask = UIViewAutoresizingFlexibleWidth; self.searchBar.placeholder = @"البحث في الملفات..."; self.searchBar.searchBarStyle = UISearchBarStyleMinimal; self.searchBar.tintColor = [UIColor colorWithRed:1 green:.22 blue:.18 alpha:1]; self.searchBar.semanticContentAttribute = UISemanticContentAttributeForceRightToLeft; [self.dashboardHeader addSubview:self.searchBar];
    UIButton *filter = [UIButton buttonWithType:UIButtonTypeSystem]; filter.frame = CGRectMake(24, 94, 88, 38); filter.layer.cornerRadius = 14; filter.layer.borderWidth = 1; filter.layer.borderColor = [UIColor colorWithWhite:1 alpha:.12].CGColor; [filter setTitle:@"☷  تصفية" forState:UIControlStateNormal]; filter.titleLabel.font = [UIFont systemFontOfSize:14 weight:UIFontWeightSemibold]; [filter setTitleColor:[UIColor colorWithRed:1 green:.28 blue:.23 alpha:1] forState:UIControlStateNormal]; [self.dashboardHeader addSubview:filter];
    UIView *stats = [[UIView alloc] initWithFrame:CGRectMake(24, 139, MAX(width - 48, 272), 88)]; stats.autoresizingMask = UIViewAutoresizingFlexibleWidth; stats.backgroundColor = [UIColor colorWithRed:.065 green:.066 blue:.075 alpha:1]; stats.layer.cornerRadius = 20; stats.layer.borderWidth = 1; stats.layer.borderColor = [UIColor colorWithRed:.42 green:.08 blue:.09 alpha:.65].CGColor; [self.dashboardHeader addSubview:stats];
